...
As described earlier, \gls{dslacr}s can also be declarative or imperative. The former describe the state, while the latter describe state changes. Because \gls{iacacr} always aims at describing the whole state, declarative languages are better fitted for this task \hl{Infrastructure as Code, Kief Morris}. They also have the property of being idempotent: If applied multiple times, the result won't change \hl{Infrastructure as Code, Kief Morris}.
Another view on these two types is that imperative code describes a set of instructions that specifies how to make things happen, while declarative code describes the desired state \hl{Infrastructure as Code, Kief Morris, second edition}. And describing the desired state is exactly the purpose of \gls{iacacr}.
\newline
Therefore, all languages that are imperatively describing infrastructure can be ruled out. Funnily enough, this doesn't apply to any of the introduced \gls{dslacr}s.
\newline
...
\newline
As already stated earlier, Amazon CloudFormation is too AWS-specific to be easily extensible to bare-metal and is therfore also not an option.
\newline

%\newline
%TODO culling: Heat, OCCI
% Heat: badly documented, mixed documentation for the cloudformation style and hot, few examples, huge ecosystem required to run even basic provisioning
% OCCI: see paper, badly documented, few examples, no real-world example/implementation (?)

\section{Approach}
Comparing languages and selecting the best one is a hard task. Not only is it hard to agree on what is important to compare, nor is it just time-consuming, but it is greatly domain-specific as well.
...
The languages this thesis is going to compare are OpenStack Heat/HOT, HashiCorp Configuration Language (HCL), Pulumi (Golang-SDK), OCCI and TOSCA with two subprofiles \textquote{Simple-Profile} and \textquote{Cloudify}.
...
\begin{table}[H]
  ...
  \begin{tabular}{ | l | l | l | l | l | }
    \hline
    Dimension & Heat/HOT & HCL/Terraform & Pulumi & TOSCA Simple-Profile \\
    \hline
    Approach & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    % Abstraction level & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    % Isomorphism & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Guarantees & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Reusability & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Errorproneness & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Amount of code & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Aspects to learn & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Viscosity & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Dependencies & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Visibility & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Extensibility & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Maturity & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    Ecosystem & \char`\~ &\char`\~ &\char`\~ &\char`\~ \\
    \hline
  \end{tabular}
  % }
