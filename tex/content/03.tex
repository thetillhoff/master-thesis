...
\section{Selection}
Since previously, many \gls{dslacr}s were ruled out, this thesis is going to look at only three languages in more detail. These are HashiCorp Configuration Language (HCL), Pulumi (Golang-SDK) and TOSCA in combination with the \textquote{Simple-Profile} extension.
\newline
%The following text and the table should be after the text for all three languages
...
\newline
In larger environments, making the correct change to the infrastructure described with \gls{hclacr} can be hard. Not only does the developer need to know all of the used technologies/platforms/providers used in the state description, but he has to find which one applies to the desired bit as well.
